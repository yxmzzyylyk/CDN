...
function wpcom_sc_icon($atts){
    if($atts['name']){
        $html = WPCOM::icon($atts['name'], false);
        return $html;
    }
}
    ...
    if($content){
        ...
        if(isset($atts['icon']) && $atts['icon']){
            if(isset($atts['size']) && $atts['size'] == '1'){
                $html .= (WPCOM::icon($atts['icon'], false, 'fa-lg') . ' <div class="alert-content">'.do_shortcode($content).'</div>');
            }else{
                $html .= (WPCOM::icon($atts['icon'], false) . ' ' . do_shortcode($content));
            }
        }else{
...
function wpcom_sc_panel($atts, $content){
    ...
    if($data && isset($data['body'])){
        ...
        if(isset($data['title']) && $data['title']){
            $html .= '<div class="panel-heading"><h3 class="panel-title">'.(isset($data['icon'])&&$data['icon'] ? WPCOM::icon($data['icon'], false) . ' ' : '').$data['title'].'</h3></div>';
        }
