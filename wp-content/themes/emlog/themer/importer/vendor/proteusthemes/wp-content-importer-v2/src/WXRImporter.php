...
class WXRImporter extends \WP_Importer {
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
            ...
            if(preg_match('/^\/\//i', $remote_url)) $remote_url = 'http:' . $remote_url;
	...
	protected function prefill_existing_posts() {
		global $wpdb;
		// wpcom：增加LIMIT限制，防止太大内存不足
		$posts = $wpdb->get_results( "SELECT ID, guid FROM {$wpdb->posts} LIMIT 1000" );
	...
	protected function prefill_existing_comments() {
		global $wpdb;
        // wpcom：增加LIMIT限制，防止太大内存不足
		$posts = $wpdb->get_results( "SELECT comment_ID, comment_author, comment_date FROM {$wpdb->comments} LIMIT 2000" );
