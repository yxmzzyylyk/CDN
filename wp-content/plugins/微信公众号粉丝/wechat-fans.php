...
function huoduan_wechat_fans($content){
	...
	if (preg_match_all('/<!--wechatfans start-->([\s\S]*?)<!--wechatfans end-->/i', $content, $hide_words))
	{
		...
		if($cookievalue==$cv){
			$content = str_replace($hide_words[0], '<div style="border:1px dashed #009900; padding:10px; margin:10px 0; line-height:200%;  background-color:#fbfafa; overflow:hidden; clear:both;">'.$hide_words[0][0].'</div>', $content);	
		}else{
			
			$hide_notice = '<h3>下载地址</h3><p></p><div class="huoduan_hide_box" style="border:2px dashed #F60; padding:5px; margin:10px 0; line-height:200%; color:#2196f3; background:url(https://p7.itc.cn/images03/20201216/770597b3cdf94398857035b536f9183b.gif);    overflow: hidden;
 clear:both;    background-size: 30.904%;"><img class="wxpic" align="right" src="'.$wechatfans['wechat_qrimg'].'" style="width:96px;height:96px;margin-left:20px;display:inline;border:none" width="150" height="150"  alt="'.$wechatfans['wechat_name'].'" /><span style="color:#f74848;font-size: 16px;">为了防止采集站，部分内容已经被隐藏，输入暗号后即可查看</span><form method="post" style="margin:10px 0;"><span class="yzts" style="font-size:16px;float:left;color:#FF6600;">暗号：</span><input name="huo'.'duan_verifycode" id="verifycode" type="text" value="" style="border:none;float:left;width:70px; height:32px; line-height:30px; padding:0 5px; border:1px solid #FF6600;-moz-border-radius: 0px;  -webkit-border-radius: 0px;  border-radius:0px;" /><input id="verifybtn" style="border:none;float:left;width:80px; height:32px; line-height:32px; padding:0 5px; background-color:#FF6600; text-align:center; border:none; cursor:pointer; color:#FFF;-moz-border-radius: 0px; font-size:14px;  -webkit-border-radius: 0px;  border-radius:0px;" name="" type="submit" value="提交查看" /></form><div style="clear:left;"></div><span style="color:#00BF30">微信扫描右侧二维码，回复关键词“<span style="color:blue">'.$wechatfans['wechat_keyword'].'</span>”，获取暗号后在此处输入即可</div>'.$vtips;
